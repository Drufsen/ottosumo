...
// ---- DRIVE PARAMETERS ----
#define L_MOTOR_SPEED 255 //Left motor speed, value between 0 (stoped) - 255 (max)
#define R_MOTOR_SPEED 255 //Right motor speed, value between 0 (stoped) - 255 (max)
#define STEER_CLEAR_REVERSE_TIME 400 //Defines how long in ms (and therefor how far) the robot should reverse while clering the border
#define STEER_CLEAR_TURN_TIME 400 //Defines how long in ms (and therefor how many degrees) the robot shuld turn when it is manovering away from the border
#define SLOW_SPEED_FACTOR 3 //Soemtimes one of or both wheels will drive in slow mode, in this case the speed will be X_MOTOR_SPEED/SLOW_SPEED_FACTOR
...
#define MAX_SEARCH_DISTANCE 35 //Defines how far away (in cm) the rebot will look for other robots, a to large value might cause the robot to chase objects outside of the arena
...
class Robot{ //The robot is controlled by one object "otto" created by this class
  public:
    ...
    void controller(); //Drives the robot based on previous data and sesnor input
  private:
    //Functions
    ...
    bool is_borders_clear(); //Reads the line sensors and returns true if no borders is foud, otherwise false
    ...
    char driving_mode; //Stores the mode the robot is in: 'h' = halted stopped, 'b' = avoiding Border, 's' = searching, 't' = tracking oponent
    
    //Data members used while steering clear ofborder
    ...
    unsigned long long search_timer = 0; //Used to keep track of how long the robot has been rotationg or driving while searching for other robot
